async function identify_user(agent)
  {
    const acct_num = agent.parameters.phoneNo;
    let client = new MongoClient(url,{useNewUrlParser : true, useUnifiedTopology :true});
    await client.connect();
    
    let snap = await client.db("Chatbot(Project_2)").collection("User_Details").findOne({"PhoneNo": acct_num});
  ...
  function report_issue(agent)
  {
    ...
    const intent_val=agent.parameters.option;
    ...
    if (err) throw err;
    var dbo = db.db("Chatbot(Project_2)");
...
module.exports = { identify_user : identify_user , report_issue : report_issue , custom_payload : custom_payload};